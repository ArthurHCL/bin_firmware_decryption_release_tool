...
int bin_firmware_decryption_and_download(void)
{
...
is_reconnect_needed:
	...
	*(unsigned int *)&hid_data_buf[6] = calculated_permission_key;
	...
	/* open bin file of released firmware. */
	released_firmware_bin_file = fopen(released_firmware_bin_file_name, "rb");
	if (NULL == released_firmware_bin_file) {
		perror("fopen() of released bin file");
	...
	while (!is_released_firmware_bin_file_read_complete) {
		...
		*/
		if (read_items_amount) {
			...
			hid_data_buf[2]                     = (unsigned char)read_items_amount;
			*(unsigned short *)&hid_data_buf[3] = frame_number++;
			...
			process_bar = (unsigned char)(100 * read_items_amount_downloaded / original_firmware_bin_file_header.firmware_length);
		}
	}
